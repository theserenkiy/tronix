...
export default {
	...
	watch:{
		'env.settings':{
			handler(){
				Math.round(env.settings.frame_height_dots*0.75);
			},
			deep:true
		}
	},
	computed:{
		...
		c_default_width(){
			let b = Math.round(env.settings.frame_height_dots*0.75)
			return this.font.default_width || b;
		},

		c_glyphStyle(){
			let s = env.settings;
			return {
				height: (s.editor_pixel_fullsize*s.frame_height_dots+40)+'px'
			}
		},
	...
	<div class=ed_fonts>
		...
		<div class=controls>
			<h2>Шрифт "{{name}}"</h2>
			<div>
				Ширина по умолч.:&nbsp;<input type="number" min="1" max="32" :value="c_default_width" @change="setDfltWidth">
			</div>
			<button 
