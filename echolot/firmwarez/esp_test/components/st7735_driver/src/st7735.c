...
#define VMEM_SZ	ST7735_WIDTH * ST7735_HEIGHT
...
static inline void color_swap(uint16_t *color)
{
...
static void fill_row(uint16_t *row, uint16_t sz, uint16_t color)
{
	for(int i=0;i < sz;i++)
		row[i] = color;
}
...
void st7735_fill_rect(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t color) {
	...
	for(int row=0;row < h;row++)
	{
		if(!row)
		{
			fill_row(start,w,color);
		}
		else
		{
			memcpy((void*)cur,(void*)start,w*2);
		}
		cur += display_width;
	}
}
	...
	for (uint8_t col = 0; col < 5; col++) {
		...
		for (uint8_t row = 0; row < 7; row++) {
			...
			if(line & (1 << row))
			{
				...
				else st7735_fill_rect(x + col*size, y + row*size, size, size, color);
			}
		}
	}
}
...
void st7735_draw_string(uint16_t x, uint16_t y, const char *str, uint16_t color, uint8_t size) {
	uint16_t cx = x;
	uint8_t char_w = 6*size;
	uint8_t char_h = 8*size;
	while (*str) {
		if (*str == '\n') { y += char_h; cx = x; }
		else { 
			st7735_draw_char(cx, y, *str, color, size); 
			cx += char_w; 
			if(cx > display_width-char_w){
				y += char_h; 
				cx = x; 
			}
		}
